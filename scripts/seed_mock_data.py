...
def seed_data():
    with app.app_context():
        ...
        # models.InterviewExperience.query.delete()
        # models.Company.query.delete()
        # models.User.query.delete()
        # db.session.execute('SET FOREIGN_KEY_CHECKS = 1')
        # db.session.commit()

        # 1. Create a mock Alumni user if not exists
        ...
        # 3. Insert Companies and Experiences
        for comp_data in mock_companies:
            ...
            for exp_data in comp_data['experiences']:
                ...
                if not existing_exp:
                    experience = models.InterviewExperience(
                        ...
                        brief=exp_data['brief'],
                        application_process=exp_data['application_process'],
                        my_experience=exp_data['my_experience'],
                        interview_rounds=exp_data['interview_rounds'],
                        technical_questions=exp_data['technical_questions'],
                        behavioral_questions=exp_data['behavioral_questions'],
                        mistakes=exp_data['mistakes'],
                        preparation_strategy=exp_data['preparation_strategy'],
                        final_advice=exp_data['final_advice'],
                        is_approved=True # Automatically approve mock data
                    )
                    db.session.add(experience)
                    print(f"Added experience for {company.name} by {mock_alumni.first_name}")
